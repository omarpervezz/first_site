...
/*! popup.js */
$(document).ready(function(){$('.te-header__dropdown__link').click(function(){$('body').toggleClass('te-header__dropdown_opened');$('body').removeClass('te-header__bookmarks_opened')})});$(document).ready(function(){$('.te-header__bookmarks__link').click(function(){$('body').toggleClass('te-header__bookmarks_opened');$('body').removeClass('te-header__dropdown_opened')})});$(document).ready(function(){$(".te-header__usermenu").hover(function(){$(".te-header__usermenu").toggleClass("te-header__usermenu__active")})});$(document).ready(function(){$(window).scroll(function(){var scroll=$(window).scrollTop();if(scroll>=100){$(".te-grid__topslide-arrow__wrapper").addClass("te-grid__topslide-arrow__wrapper_hidden")}})})
/*! education.js */
$(document).ready(function(){window.keyUpFunc=function(e){if(e.keyCode==27){$('.js-te-popup-close').click()}}
$('.js-te-popup-close').click(function(){$('body').removeClass('te-body_popup-opened');var popup=$(".te-popup_opened");popup.fadeOut();$(document).unbind("keyup",keyUpFunc);return!1});$('body').on('click','.js-te-popup-show',function(){var popup_div_id='#popup_'+$(this).data('popup-id');var $popup=$(popup_div_id);var desc=$(this).data('popup-description');$('body').addClass('te-body_popup-opened');if(desc&&desc>''){$popup.find('.te-popup__description').html(desc)}
$popup.addClass('te-popup_opened').show();$popup.fadeIn('fast');$(document).keyup(keyUpFunc);var target=$(this).attr('href');if(target&&target.substring(0,1)=='#'){$popup.data('target',target)}
if($(this).data('show-callback')>''){var funcname='window.'+$(this).data('show-callback')+'Callback';eval(funcname+'($popup, $(this))')}
return!1});var heightForTopSlide;if($('.te-grid__topslide').parent().hasClass('te-grid__container-withoutmenu')){heightForTopSlide=0}else{heightForTopSlide=55}
$('.te-grid__topslide').css({'height':(($(window).height()-heightForTopSlide))+'px'});$(window).resize(function(){$('.te-grid__topslide').css({'height':(($(window).height()-heightForTopSlide))+'px'})});if(window.$isMobile==undefined){window.$isMobile=!1;isWidthLimited=$(window).width()<=1024;if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){window.$isMobile=!0}}
window.showLoadIcon=function($form,text)
{var mess=text||"sending data…";var data="<div id='loadingbox' style='margin:5px 0px; font-size:14px; text-align:left;'>";data+="<img src='/img/ajax-loader.gif' style='margin-right: 10px;'>";data+=""+mess+"";data+="</div>";$form.find('#loadingbox').remove();var $button=$form.find('button[type=submit]');if($button.length!=1){$button=$form.find('input[type=submit]')}
if($button.length!=1){alert('Error! Cannot find submit button');console.log('(loadicon) Cannot find submit button in form #'+$form.attr('id'));return!1}
$button.hide().after(data)};window.hideLoadIcon=function(){var $loadbox=$('#loadingbox');$form=$loadbox.closest('form');$loadbox.remove();var $button=$form.find('button[type=submit]');if($button.length!=1){$button=$form.find('input[type=submit]')}
if($button.length!=1){alert('Error! Cannot find submit button');console.log('(hideicon) Cannot find submit button in form #'+$form.attr('id'));return!1}
$button.show()};$('.js-share').click(function(){var provider=$(this).data('socnet');var provider_url;var provider_w=500;var provider_h=500;var url=window.location.href;if(provider=='fb'){provider_w=600;provider_url='https://www.facebook.com/sharer.php?u='+url}
if(provider=='vk'){provider_url='https://vk.com/share.php?url='+url}
if(provider=='twitter'){provider_url='https://twitter.com/intent/tweet?status='+url}
myWin=window.open(provider_url,"displayWindow","width="+provider_w+",height="+provider_h+",left="+parseInt(($(window).width()-provider_w)/2)+",top="+parseInt(($(window).height()-provider_h)/2)+",status=no,toolbar=no,menubar=no");return!1});$('body').on('click','.js-toggle',function(){var id=$(this).data('togglebox');var text=$(this).data('toggletext');var textold=$(this).text();var $box;if(id>''&&$(id).length>0){$box=$(id);if($box.css('display')=='none'){$box.show();var off=$box.offset();window.scroll(0,off.top)}else{$box.hide()}
if(text>''){$(this).html(text);$(this).data('toggletext',textold)}}
return!1})})